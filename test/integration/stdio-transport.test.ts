/**
 * Integration tests for setupMcpTransports() with stdio transport
 *
 * Validates @mcp-z/server's stdio transport orchestration by:
 * - Spawning echo server using setupMcpTransports()
 * - Connecting MCP clients via stdio
 * - Testing tool/resource/prompt registration
...
describe('stdio transport', () => {
  ...
  before(async () => {
    // Spawn echo server using @mcp-z/server's setupMcpTransports()
    ...
    cluster = createServerRegistry(config, { cwd: process.cwd() });
  });
